...
Deterministic geometric distance inference from trigonometric parallax,
using a smooth prior and an adaptive two-stage 1D posterior integration.

Originally developed for the CNS6 catalogue (Catalogue of Nearby Stars,
6th edition), but general-purpose: any parallax + uncertainty in mas.

Method
------
Two-stage adaptive log-r grid:

1. Broad log-r pass to locate posterior support safely, with automatic
   upper-boundary expansion for low-S/N or negative-parallax cases.
2. Refined log-r pass concentrated around the posterior mass, preserving
   accuracy for the narrow posteriors of very nearby high-S/N stars.

This preserves sensible low-S/N behaviour while recovering accurate formal
uncertainties for precise nearby stars — without MCMC.

Output philosophy
-----------------
- Distances are geometric posteriors from parallax + prior only.
- Distances are NOT truncated at any horizon.
- P(r < threshold_pc | parallax, sigma) is computed as a separate
  posterior probability, useful for volume-limited sample membership.

Usage (CLI)
-----------
    plx2dist --input catalogue.vot --output-prefix results --priors edsd

Usage (Python API)
------------------
    from plx2dist import derive_distances
    result_df = derive_distances(df, plx_col="parallax", err_col="parallax_error")
