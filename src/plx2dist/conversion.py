...
def parallax_to_distance(parallax: Quantity) -> Quantity:
    """Convert parallax to distance via naive inversion.
    ...
    Notes
    -----
    This is a simple 1/p conversion. It does not apply priors or account
...
def log_likelihood(r: np.ndarray, w_obs: float, w_err: float) -> np.ndarray:
    """Gaussian likelihood in parallax space.
...
def summarize_distance_posterior(
    ...
    # If refinement clipped non-negligible mass, fall back to broad grid
    lost_low = cdf[0]
    lost_high = 1.0 - cdf[-1]
